...
DEV='cuda'; OUT='/workspace/optics'; t0=time.time()
...
depths=[8,12]; seeds=[0,1,2]; gammas=[0.0,0.02,0.05,0.1,0.2,0.3,0.5]
    ...
    for aname,afn in acts.items():
        ...
        for seed in seeds:
            ...
            for nm in ['const']:
                ...
                per[nm].append(curve)
            print(f"d{depth}_{aname} s{seed}: g2={per['const'][-1][1]:.4f} g5={per['const'][-1][2]:.4f} g10={per['const'][-1][3]:.4f} g20={per['const'][-1][4]:.3f} clean={per['const'][-1][0]:.3f} [{time.time()-t0:.0f}s]",flush=True)
        for nm in ['const']:
            arr=np.array(per[nm]); results['runs'][f"d{depth}_{aname}_{nm}"]={'mean':arr.mean(0).tolist(),'std':arr.std(0).tolist()}
        json.dump(results,open(os.path.join(OUT,'deep_noise_deep.json'),'w'),indent=2)
